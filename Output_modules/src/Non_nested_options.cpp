    ...
    namespace options {
        ...
        void remove_entry(const std::string& output_config, std::string::size_type* position, std::string* output_data, std::string::size_type* output_data_position) {
            try {
                ...
                non_term_entry = helper_templates_for_options::return_non_terminal_entry<search>(output_config, position, obj->list_of_entries_to_find_it_in);
                }
                else{
                non_term_entry = helper_templates_for_options::return_non_terminal_entry<search>(*output_data, output_data_position, obj->list_of_entries_to_find_it_in);
                }
       ...
       using ternary_state=uint8_t;
        ...
        template<char...>
        constexpr ternary_state operator"" _nuteral() {
            return 1;
        }
        template<char...>
...
void escape_charactor(const std::string& output_config, std::string::size_type* position, std::string* output_data, std::string::size_type* output_data_position) {
    ...
    {[&char_to_escape, size_of_string=escape_charactor.length() ]( std::string*input_string, std::string::size_type*where_is_it_found){ 
        ...
        [&slash_alternative, size_of_string=doube_slash_alternative.length()](std::string*input_string, std::string::size_type*where_is_it_found){ 
